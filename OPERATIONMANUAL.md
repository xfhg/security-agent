...
**What works offline:**
- All native scanners: Cognium, OpenGrep, poltergeist (secrets), codetree, GitNexus
- All Ghost skills in direct-execution mode: repo-context, scan-code, scan-deps, scan-secrets, report (no subagent spawning required)
- Ghost scan-deps: online-first, falls back to offline if network unavailable. offline mode requires pre-downloaded OSV database via `wraith download-db`
...
- MCP tools: `agent-harness-kit`, `codetree` (deep-dive on symbols/files), filesystem, GitNexus (graph/flow context), Semble (MCP-only, targeted retrieval), through `bins/shims/*`
...
4. safe Ghost evidence checks against `TARGET_REPO`: invoke `skill("ghost-repo-context")`, `skill("ghost-scan-deps")`, etc. — Ghost skills are OpenCode skills, NOT CLI binaries. All Ghost output goes to `scans/<reponame>/evidence/ghost/`.
5. recon with default supporting-tool preparation, which imports Ghost repo context and prepares GitNexus/code graph evidence
...
- critical external tools are separate harness gates: codeTree, GitNexus, Ghost, OpenGrep, and Cognium
...
- GitNexus indexes the target and writes graph/query evidence. Retries with `--skip-git` on non-git-repo errors.
...
- codeTree: `evidence/graph/codetree-structure.json` and security/heuristic analysis artifacts or blocker
...
3. `evidence/graph/codetree-structure.json`
4. `evidence/graph/codetree-security-symbols.json`
5. `evidence/graph/codetree-graph-context.json`
6. `workflow/recon-summary.md`
7. `kb/entrypoints.json`
8. `kb/dependencies.json`
9. `evidence/graph/gitnexus-query.json`
10. `findings/normalized/findings.json`
11. `findings/triaged/findings.json`
12. `security/executive-summary.md`
13. `security/detailed-report.md`
14. `review/rescore-report.md`
...
```text
Read repo-map, entrypoints, GitNexus summaries, then inspect only targeted snippets.
```
...
- Missing OpenGrep, Cognium, GitNexus, codeTree, AHK, filesystem MCP, or Ghost workflow evidence makes the default run fail as `coverage_incomplete`.
