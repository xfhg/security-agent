...
export async function initStage(repo: string): Promise<void> {
  ...
  await writeJson(agentPath(repo, "config", "tools.json"), {
    ...
    ghost: { enabled: true, mode: "local-artifact-import-only", skills_repo_path: DEFAULT_GHOST_SKILLS_REPO_PATH, evidence_dir: "scans/<reponame>/evidence/ghost", bins_dir: "bins/ghost", import_markdown_findings: true, normalize_to_canonical_schema: true, allow_live_validation: false, allow_proxy: false, default_in_complete_workflow: true, forbidden_external_paths: ["global-user-ghost-home"] }
  });
