...
async function probeGhostGates(repo: string): Promise<CoverageGate[]> {
  ...
  return [
    ...
    await ghostGate(repo, "ghost-deps",
      preflightOk ? depsEvidence : (deps.length > 0 || await artifactExists(repo, ["findings/normalized/ghost-deps-findings.json"])),
    ...
    await ghostGate(repo, "ghost-secrets",
      preflightOk ? secretsEvidence : (secrets.length > 0 || await artifactExists(repo, ["findings/normalized/ghost-secrets-findings.json"])),
