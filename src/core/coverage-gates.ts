...
async function probeGhostGates(repo: string): Promise<CoverageGate[]> {
  ...
  return [
    ...
    await ghostGate(repo, "ghost-deps",
      preflightOk ? depsEvidence : (depsEvidence || await artifactExists(repo, ["findings/normalized/ghost-deps-findings.json"])),
    ...
    await ghostGate(repo, "ghost-secrets",
      preflightOk ? secretsEvidence : (secretsEvidence || await artifactExists(repo, ["findings/normalized/ghost-secrets-findings.json"])),
    ...
    await ghostGate(repo, "ghost-scan-code",
      preflightOk ? (code.length > 0 || codeEvidence) : (codeEvidence || await artifactExists(repo, ["findings/normalized/ghost-code-findings.json"])),
      ["findings/normalized/ghost-code-findings.json"],
      await resolveReason(codeEvidence, ["findings/normalized/ghost-code-findings.json"], codeEvidence ? null : "Ghost code scan execution artifact missing")),
